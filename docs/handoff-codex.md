...
- **本文で使う部品:** `<KeyPoints>`（とはの末尾の要点）、`<TastingNote way rating image>`（飲み方別）、`<Rating>`（点数表）、`<ProsCons>`（おすすめする人・しない人／良い声・気になる声）、`<AffiliateBox>`（主商品以外の購入ボックス）。実体は `src/components/pages/posts/post-content/parts/`。
